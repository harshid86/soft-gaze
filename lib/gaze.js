...
function Gaze (patterns, opts, done) {
  ...
  opts.forcePolling = opts.forcePolling || false;
  this.options = opts;
...
Gaze.prototype._observeFile = function (file, done) {
  ...
  if (!this._pollers[file]) {
    ...
    try {
      fs.watch(file, opts, this._pollers[file]);
    } catch (err) {
      return this._handleError(err);
    }
  }
...
Gaze.prototype._initWatched = function (done) {
  ...
  helper.forEachSeries(curWatched, function (dir, next) {
    ...
    files.forEach(function (file) {
      ...
      var react = function (err, filepath) {
        if (err) {
          self.emit('error', err);
          return;
        }
      ...
      if (self.options.forcePolling) {
        self._pollFile(file, react);
      }
      else {
        self._observeFile(file, react);
      }
    });
